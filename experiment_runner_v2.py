...
def create_mnist_dataset(data_path, batch_size=32, is_training=True, num_samples=None):
    ...
    dataset = dataset.map(
        operations=vision.Resize((32, 32)),
        input_columns='image'
    )
    
    dataset = dataset.map(
        operations=transforms.TypeCast(mindspore.float32),
        input_columns='image'
    )
    
    dataset = dataset.map(
        operations=vision.Normalize(
            mean=(0.1307,),
            std=(0.3081,)
        ),
        input_columns='image'
...
def train_and_evaluate(model, train_dataset, test_dataset, epochs, learning_rate, 
                       optimizer_type='SGD', verbose=False):
    """
    训练和评估模型
    """
    ...
    if verbose:
        print(f"训练完成！最终测试准确率: {final_test_acc:.4f}, 耗时: {elapsed_time:.2f}s")
...
def run_experiments():
    ...
    train_dataset_base = create_mnist_dataset(data_path, batch_size=32, is_training=True, num_samples=5000)
    test_dataset_base = create_mnist_dataset(data_path, batch_size=32, is_training=False, num_samples=1000)
    ...
    for exp_name, exp_config in experiments.items():
        ...
        for result in exp_results:
            print(f"第 {result['group']} 组: 测试准确率 = {result['final_test_accuracy']:.4f}, "
